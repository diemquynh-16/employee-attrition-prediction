...
| Thông tin | Giá trị |
|---|---|
| Tên | IBM HR Analytics Employee Attrition Dataset |
| Số dòng | 1.470 |
| Số cột | 13 |
...
| `WorkLifeBalance` | Số nguyên | Mức độ cân bằng công việc - cuộc sống (1–4) |
...
- Biến số (liên tục): 5 cột (`Age`, `DistanceFromHome`, `MonthlyIncome`, `NumCompaniesWorked`, `YearsAtCompany`)

- Biến có thứ tự: 4 cột (`Education`, `EnvironmentSatisfaction`, `JobSatisfaction`, `WorkLifeBalance`)

- Biến chữ phân loại: 3 cột (`Department`, `EducationField`, `MaritalStatus`)
...
![Biểu đồ phân bố biến mục tiêu](outputs/figures_eda/eda_01_target_distribution.png)

**Nhận xét:** Dataset bị ***mất cân bằng lớp nghiêm trọng*** - tỷ lệ nhân viên nghỉ việc chỉ chiếm ~16%. Đây là đặc điểm mang tính thực tế đối với bài toán dự đoán nghỉ việc. Sự mất cân bằng này ảnh hưởng đáng kể đến kết quả mô hình nên cần phải ưu tiên chỉ số đánh giá là F1-score thay vì Accuracy.
...
Từ các biểu đồ trực quan theo Attrition có thể thấy:
...
- Nhân viên có mức độ `JobSatisfaction` (hài lòng với công việc) và `EnvironmentSatisfaction` (hài lòng với môi trường làm việc) thấp có xu hướng nghỉ việc nhiều nhất.
...
- Chuẩn hóa chuỗi: Bỏ khoảng trắng đầu/cuối cho các tên cột và dữ liệu kiểu object.
...
| KNN | `n_neighbors=7` |
| Decision Tree | `max_depth=5`, `max_leaf_nodes=10`, `class_weight="balanced"` |
...
> Lưu ý: Dataset bị mất cân bằng lớp nghiêm trọng nên cần phải ưu tiên chỉ số đánh giá là F1-score và ROC-AUC.
...
- KNN dễ bị ảnh hưởng khi tỷ lệ lớp mất cân bằng và không hỗ trợ `class_weight`.

#### Decision Tree (max_depth=5)

- Test F1 = 0.2381, Test Recall = 0.3191
- Train F1 (0.48) cao hơn Test F1 (0.301) => có dấu hiệu overfitting rõ rệt và khả năng tổng quát hóa  trên dữ liệu mới còn kém
- ROC-AUC thấp (0.57) - gần với mô hình đoán ngẫu nhiên

#### Random Forest

- Accuracy test cao (0.8367) nhưng Test F1 chỉ đạt 0.200.
...
- 75 trường hợp False Positive (dự đoán nghỉ nhưng thực ra ở lại) - chi phí thấp (có thể can thiệp giữ chân không cần thiết).
...
| Random Forest | Overfitting nghiêm trọng - Train F1=0.98 nhưng Test F1=0.20 |
...
2. F1-score tổng thể thấp: Ngay cả mô hình tốt nhất chỉ đạt Test F1 = 0.35.
...
3. Thử các mô hình mạnh hơn như GradientBoostingClassifier hoặc XGBoost, thường cho kết quả tốt hơn trên dữ liệu mất cân bằng.

4. Điều chỉnh threshold phân loại: Thay vì dùng ngưỡng 0.5 mặc định, hạ threshold xuống (ví dụ 0.3) để tăng Recall - phù hợp với mục tiêu nghiệp vụ là phát hiện sớm nhân viên có nguy cơ nghỉ.
